-- Required: cannot change return type with CREATE OR REPLACE. Must DROP then CREATE.
drop function if exists public.get_entitlements();
-- Make entitlements reflect an active trial as (tier=core, status=trialing),
-- even if subscription_tier/status still say free/active.
-- Also backfill tenant rows for active trials.

-- 1) Backfill tenant rows where trial window is active
update public.tenants t
set
  subscription_status = 'trialing'::public.subscription_status,
  subscription_tier   = 'core'
where
  t.trial_started_at is not null
  and t.trial_ends_at is not null
  and now() < t.trial_ends_at
  and (t.subscription_status <> 'trialing'::public.subscription_status or t.subscription_tier <> 'core');

-- 2) Replace get_entitlements() to compute effective tier/status from trial window
create function public.get_entitlements()
returns table (
  tenant_id uuid,
  workspace_name text,
  role text,
  tier text,
  status text,
  trial_ends_at timestamptz
)
language sql
stable
security definer
set search_path = public, auth, extensions
as $function$
  select
    tm.tenant_id,
    t.workspace_name,
    tm.role,
    case
      when t.trial_started_at is not null and t.trial_ends_at is not null and now() < t.trial_ends_at then 'core'
      else coalesce(t.subscription_tier, 'free')
    end as tier,
    case
      when t.trial_started_at is not null and t.trial_ends_at is not null and now() < t.trial_ends_at then 'trialing'
      else coalesce(t.subscription_status::text, 'pending')
    end as status,
    t.trial_ends_at
  from public.tenant_memberships tm
  join public.tenants t on t.id = tm.tenant_id
  where tm.user_id = auth.uid()
  order by tm.created_at desc nulls last;
$function$;

grant execute on function public.get_entitlements() to authenticated;

-- 3) Force PostgREST schema reload
do $function$
begin
exception when others then
  null;
end $function$;